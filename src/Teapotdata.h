...
const static uint16 kTeapotNumVertices = 306;

uint16 teapotPatches[kTeapotNumPatches][16] = {
...
xm::vec3 teapotVertices[kTeapotNumVertices] = {
...
vec evalBezierCurve(vec P[4], float t)
{
	vec a = P[0] + (P[1] - P[0]) * t;
	vec b = P[1] + (P[2] - P[1]) * t;
	vec c = P[2] + (P[3] - P[2]) * t;
...
template <typename vec>
vec evalBezierSurface(vec P[16], float u, float v)
{
	vec Pu[4];
	for (int i = 0; i < 4; ++i)
	{
...
SimpleMesh generateTeapotMesh(uint accuracy)
{
	SimpleMesh teapot;

	teapot.m_vertices.reserve((accuracy + 1) * (accuracy + 1) * kTeapotNumPatches);
	teapot.m_indices.reserve(accuracy * accuracy * 2 * kTeapotNumPatches);

	xm::uvec3 bot_idx(0, 1, 4);
	xm::uvec3 top_idx(1, 5, 4);

	for (uint16 p = 0; p < kTeapotNumPatches; ++p)
	{
		xm::vec3 points[16];
		for (uint16 i = 0; i < 16; ++i) 
		{
			points[i] = teapotVertices[teapotPatches[p][i] - 1];
		}

		for (uint u = 0; u < accuracy + 1; ++u)
		{	
			for (uint v = 0; v < accuracy + 1; ++v)
			{
				float uf = u / static_cast<float>(accuracy);
				float vf = v / static_cast<float>(accuracy);

				teapot.m_vertices.emplace_back(evalBezierSurface(points, uf, vf));
			}
		}

		uint m = p * (accuracy + 1) * (accuracy + 1);
		uint n = accuracy + 1;
		for (uint u = 0; u < accuracy; ++u)
		{
			for (uint v = 0; v < accuracy; ++v)
			{
				uint a = u * n + m + v;
				uint b = a + 1;
				uint c = a + n;
				...
				teapot.m_indices.emplace_back(xm::uvec3(a, b, c));
				teapot.m_indices.emplace_back(xm::uvec3(b, d, c));
			}
		}
	}
	return teapot;
